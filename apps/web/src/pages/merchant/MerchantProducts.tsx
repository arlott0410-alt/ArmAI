...
export default function MerchantProducts() {
  ...
  const validate = (): string | null => {
    ...
    if (form.currency && form.currency.length !== 3)
      return 'Currency must be 3 characters (e.g. LAK, THB).'
    return null
  }
        ...
        <CardBody style={{ padding: 0 }}>
          ...
          {filtered.length === 0 ? (
            <EmptyState
              ...
              description={
                products.length === 0
                  ? 'Add products and categories so the AI can answer accurately.'
                ...
                {filtered.map((p) => (
                  <tr key={p.id} style={{ borderBottom: `1px solid ${theme.borderMuted}` }}>
                    ...
                    <td style={{ padding: '12px 16px' }}>
                      {p.sale_price ?? p.base_price} {p.currency ?? defaultCurrency}
                    </td>
                    <td style={{ padding: '12px 16px' }}>
      ...
      >
        ...
        <FieldGroup label="Currency (3 chars, optional)">
          <input
            ...
            value={form.currency ?? ''}
            onChange={(e) =>
              setForm((f) => ({ ...f, currency: e.target.value.toUpperCase() || undefined }))
            }
            style={inputStyle}
        ...
        <FieldGroup label="Requires manual confirmation">
          <input
            ...
            onChange={(e) =>
              setForm((f) => ({ ...f, requires_manual_confirmation: e.target.checked }))
            }
            style={{ marginRight: 8 }}
          />
        ...
        <FieldGroup label="COD requires manual confirmation">
          <input
            ...
            onChange={(e) =>
              setForm((f) => ({ ...f, requires_manual_cod_confirmation: e.target.checked }))
            }
            style={{ marginRight: 8 }}
          />
          <span style={{ fontSize: 13, color: theme.textSecondary }}>
            Merchant must confirm before COD is ready to ship
          </span>
        </FieldGroup>
      </FormModal>
    </PageShell>
