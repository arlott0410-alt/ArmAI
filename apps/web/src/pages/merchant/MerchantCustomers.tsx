      ...
      >
        {customers.length === 0 ? (
          <p style={{ color: theme.textSecondary, margin: 0 }}>
            No customer profiles yet. Profiles are created when channel identities are linked (e.g.
            by phone match or manual link).
          </p>
        ) : (
                ...
                {customers.map((c) => (
                  <tr key={c.id} style={{ borderBottom: `1px solid ${theme.borderMuted}` }}>
                    ...
                    <td style={{ padding: '12px 16px' }}>
                      <span
                        style={{
                          color: c.status === 'active' ? theme.success : theme.textMuted,
                          fontSize: 12,
                        }}
                      >
                        {c.status}
