...
export default function MerchantTelegram() {
  ...
  const handleAddAdmin = async () => {
    ...
    setAddingAdmin(true)
    try {
      await telegramApi.addAdmin(token, {
        telegram_user_id: newAdminUserId.trim(),
        role: newAdminRole,
      })
      setNewAdminUserId('')
      load()
  ...
  const merchantId =
    (data as { merchantId?: string } | null)?.merchantId ?? data?.connection?.merchant_id ?? ''
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
        ...
        >
          ...
          >
            <div>
              ...
              />
              <div style={hintStyle}>
                Get the group ID by adding @userinfobot to the group or from the group invite link.
                Use the numeric ID (e.g. -100…).
              </div>
            </div>
            <div>
            ...
            <div>
              ...
              />
              <div style={hintStyle}>
                From @BotFather. Stored securely; never shown again after save. Leave blank to keep
                existing token.
              </div>
            </div>
        ...
        <PanelCard
          title="Webhook URL"
          subtitle="Set this in your bot via setWebhook so Telegram sends updates to ArmAI."
        >
          <CopyField value={webhookUrl} label="Webhook URL" />
          <div style={hintStyle}>
            POST updates to this URL. Your backend will route by merchant ID.
          </div>
        </PanelCard>
          ...
          {testResult && (
            <p
              style={{
                marginTop: 12,
                fontSize: 13,
                color: testResult.includes('sent') ? theme.success : theme.danger,
              }}
            >
              {testResult}
            </p>
          ...
          >
            {admins.map((a) => (
              <li key={a.id} style={{ marginBottom: 6 }}>
                {a.telegram_user_id} {a.telegram_username ? `@${a.telegram_username}` : ''} —{' '}
                {a.role} {!a.is_active && '(inactive)'}
              </li>
            ))}
