...
export default function MerchantOrderDetail() {
  ...
  const switchPaymentMethod = async (desiredMethod: string) => {
    ...
    setActionError(null)
    try {
      const res = await merchantApi.orderSwitchPaymentMethod(token, orderId, {
        desired_method: desiredMethod,
        requested_by: 'merchant_admin',
      })
  ...
  const codDetails = order.cod_details as {
    cod_status?: string
    cod_amount?: number
    cod_fee?: number
  } | null
  ...
  const canSwitch =
    !order.payment_method_locked_at &&
    order.status !== 'paid' &&
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
        <PanelCard title="Order" subtitle="Status and payment">
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 16, alignItems: 'center' }}>
            ...
            <div>
              ...
              <Badge variant={isCod ? 'gold' : 'default'}>
            ...
            <div>
              ...
              <span style={{ fontSize: 13 }}>{order.payment_switch_count ?? 0}</span>
            </div>
            <div>
              <span style={{ fontSize: 12, color: theme.textMuted, marginRight: 8 }}>Amount</span>
              <span style={{ fontSize: 13 }}>{order.amount ?? '—'}</span>
            </div>
            <div>
              <span style={{ fontSize: 12, color: theme.textMuted, marginRight: 8 }}>Customer</span>
              <span style={{ fontSize: 13 }}>{order.customer_name ?? '—'}</span>
            </div>
          </div>
          {canSwitch && (
            <div
              style={{ marginTop: 12, paddingTop: 12, borderTop: `1px solid ${theme.borderMuted}` }}
            >
              <span style={{ fontSize: 12, color: theme.textMuted, marginRight: 8 }}>
              ...
              {!isCod && (
                <button
                  type="button"
                  onClick={() => switchPaymentMethod('cod')}
                  style={{
                    marginRight: 8,
                    padding: '6px 12px',
              ...
              {isCod && (
                <button
                  type="button"
                  onClick={() => switchPaymentMethod('prepaid_bank_transfer')}
                  style={{
                    marginRight: 8,
                    padding: '6px 12px',
                    background: theme.surfaceElevated,
          ...
          {actionError && (
            <p style={{ color: theme.danger, fontSize: 13, marginTop: 8 }}>{actionError}</p>
          )}
        </PanelCard>
            ...
            <ul style={{ margin: 0, paddingLeft: 20, fontSize: 13, color: theme.textSecondary }}>
              {events.map((ev) => (
                <li key={ev.id} style={{ marginBottom: 6 }}>
                  {ev.from_method} → {ev.to_method} ({ev.switch_result}){' '}
                  {ev.reason ? `— ${ev.reason}` : ''} — {new Date(ev.created_at).toLocaleString()}
                </li>
              ))}
            </ul>
          </PanelCard>
        ...
        {isCod && codDetails && (
          <PanelCard title="COD details" subtitle="Cash on Delivery status">
            <div style={{ marginBottom: 12 }}>
              Status: <StatusBadge status={codDetails.cod_status ?? ''} /> Amount:{' '}
              {codDetails.cod_amount} Fee: {codDetails.cod_fee ?? 0}
            </div>
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              {codDetails.cod_status === 'pending_merchant_confirmation' && (
                <button
                  type="button"
                  onClick={() =>
                    token && orderId && runAction(() => merchantApi.orderCodConfirm(token, orderId))
                  }
                  style={{
                    padding: '8px 16px',
              ...
              {(codDetails.cod_status === 'ready_to_ship' ||
                codDetails.cod_status === 'pending_merchant_confirmation') && (
                <button
                  type="button"
                  onClick={() =>
                    token &&
                    orderId &&
                    runAction(() => merchantApi.orderCodMarkShipped(token, orderId))
                  }
                  style={{
                    padding: '8px 16px',
                    background: theme.surfaceElevated,
              ...
              {codDetails.cod_status === 'shipped' && (
                <button
                  ...
                  onClick={() =>
                    token &&
                    orderId &&
                    runAction(() => merchantApi.orderCodMarkCollected(token, orderId))
                  }
                  style={{
                    padding: '8px 16px',
              ...
              {(codDetails.cod_status === 'ready_to_ship' ||
                ...
                <button
                  ...
                  onClick={() =>
                    token &&
                    orderId &&
                    runAction(() => merchantApi.orderCodMarkFailed(token, orderId))
                  }
                  style={{
                    padding: '8px 16px',
        ...
        {(order.payment_status === 'paid' ||
          ...
          <PanelCard title="Fulfillment" subtitle="Shipment and delivery">
            ...
            >
              ...
              <span style={{ fontSize: 13 }}>{order.payment_status ?? '—'}</span>
              <span style={{ fontSize: 12, color: theme.textMuted, marginLeft: 12 }}>
                Fulfillment status
              </span>
              <FulfillmentStatusBadge status={order.fulfillment_status} />
            </div>
            {(order.shipment_images?.length ?? 0) > 0 && (
              <div style={{ marginBottom: 16 }}>
                <div
                  style={{ fontSize: 12, fontWeight: 600, color: theme.textMuted, marginBottom: 8 }}
                >
                  Shipment images
                </div>
                {(order.shipment_images ?? []).map(
                  (img: {
                    id: string
                    ...
                    <div
                      key={img.id}
                      style={{ display: 'inline-block', marginRight: 12, marginBottom: 8 }}
                    >
                      {img.image_url ? (
                        <a
                          href={img.image_url}
                      ...
                      <div style={{ fontSize: 11, color: theme.textMuted, marginTop: 4 }}>
                        Source: {img.source} · {img.processing_status}
                      </div>
                    </div>
                  )
                )}
                  ...
                  (
                    ...
                    >
                      {(s as { shipment_proof_mode?: string }).shipment_proof_mode === 'image' && (
                        <div style={{ fontSize: 11, color: theme.primary, marginBottom: 6 }}>
                          Proof: image (from Telegram or dashboard)
                        </div>
                      )}
                      {(s as { customer_notified_at?: string | null }).customer_notified_at && (
                        <div style={{ fontSize: 11, color: theme.success, marginBottom: 6 }}>
                          Customer notified
                        </div>
                      )}
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, marginBottom: 8 }}>
                        <span>
                          <strong style={{ color: theme.textMuted }}>Courier</strong>{' '}
                          {s.courier_name ?? '—'}
                        </span>
                        <span>
                          <strong style={{ color: theme.textMuted }}>Tracking</strong>{' '}
                          {s.tracking_number ?? '—'}
                        </span>
                        {s.tracking_url && (
                          <a
                        ...
                        <span>
                          <strong style={{ color: theme.textMuted }}>Shipped</strong>{' '}
                          {s.shipped_at ? new Date(s.shipped_at).toLocaleString() : '—'}
                        </span>
                        <span>
                          <strong style={{ color: theme.textMuted }}>Status</strong>{' '}
                          {s.shipment_status}
                        </span>
                      </div>
                      {s.shipping_note && (
                      ...
                      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                        {s.shipment_status !== 'shipped' && s.shipment_status !== 'in_transit' && (
                          <button
                            ...
                            onClick={() =>
                              token &&
                              runAction(() =>
                                merchantApi.updateShipment(token, s.id, {
                        ...
                        <button
                          ...
                          onClick={() =>
                            token &&
                            runAction(() => merchantApi.sendShipmentConfirmation(token, s.id))
                          }
                          style={{
                            padding: '6px 12px',
              ...
              >
                ...
                <div style={{ display: 'grid', gap: 10, maxWidth: 400 }}>
                  <input
                    ...
                    onChange={(e) =>
                      setShipmentForm((f) => ({ ...f, courier_name: e.target.value || undefined }))
                    }
                    style={{
                      padding: 8,
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setShipmentForm((f) => ({
                        ...f,
                        tracking_number: e.target.value || undefined,
                      }))
                    }
                    style={{
                      padding: 8,
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setShipmentForm((f) => ({ ...f, tracking_url: e.target.value || undefined }))
                    }
                    style={{
                      padding: 8,
                      background: theme.surface,
                  ...
                  <input
                    ...
                    value={shipmentForm.shipping_note ?? ''}
                    onChange={(e) =>
                      setShipmentForm((f) => ({ ...f, shipping_note: e.target.value || undefined }))
                    }
                    style={{
                      padding: 8,
                      background: theme.surface,
            ...
            {order.fulfillment_events?.length || order.telegram_operation_events?.length ? (
              ...
              >
                ...
                <ul
                  style={{ margin: 0, paddingLeft: 20, fontSize: 12, color: theme.textSecondary }}
                >
                  {[
                    ...(order.fulfillment_events ?? []).map((ev) => ({
                      id: ev.id,
                      type: ev.event_type,
                      at: ev.created_at,
                      source: 'fulfillment',
                    })),
                    ...(order.telegram_operation_events ?? []).map((ev) => ({
                      id: ev.id,
                      type: ev.event_type,
                      at: ev.created_at,
                      source: 'telegram',
                    ...
                    .map((ev) => (
                      <li key={ev.id}>
                        ...
                        {ev.source === 'telegram' && (
                          <span style={{ color: theme.textMuted, marginLeft: 6 }}>(Telegram)</span>
                        )}
                        {' — '}
          ...
          <PanelCard title="Items" subtitle="">
            <ul style={{ margin: 0, paddingLeft: 20, fontSize: 13 }}>
              {order.order_items.map((item) => (
                <li key={item.id}>
                  {item.product_name_snapshot} x{item.quantity} = {item.total_price}
                </li>
              ))}
            </ul>
          </PanelCard>
        ...
        {order.shipping_details && (
          <PanelCard title="Shipping details" subtitle="">
            ...
            >
              {JSON.stringify(order.shipping_details, null, 2)}
            </pre>
          </PanelCard>
        )}
      </div>
