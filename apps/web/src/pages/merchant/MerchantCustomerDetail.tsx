...
export default function MerchantCustomerDetail() {
  ...
  const handleLink = async (channelIdentityId: string) => {
    ...
    try {
      await customerIdentitiesApi.link(token, {
  ...
  const handleUnlink = async (channelIdentityId: string) => {
    ...
    if (
      !confirm(
        'Unlink this channel identity from this customer? The identity will remain but no longer link to this profile.'
      )
    )
      return
  ...
  const customer = data.customer as MerchantCustomerDetailResponse['customer'] & {
    notes?: string | null
  }
    ...
    >
      <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
        <PanelCard title="Customer summary" subtitle="Display name, phone, status">
          ...
          >
            ...
            <div>
              ...
              <span
                style={{ color: customer.status === 'active' ? theme.success : theme.textMuted }}
              >
                {customer.status}
              </span>
            </div>
              ...
              {data.identities.map((ident) => (
                ...
                >
                  <span style={{ color: theme.text }}>
                    <strong>{ident.channel_type}</strong> —{' '}
                    {ident.channel_display_name ?? ident.external_user_id}
                  </span>
                  <span style={{ fontSize: 12, color: theme.textMuted }}>
                    Last seen {new Date(ident.last_seen_at).toLocaleString()}
                  </span>
                  <button
                    type="button"
        ...
        {suggestions.length > 0 && (
          <PanelCard
            title="Identity link suggestions"
            subtitle="Unlinked identities with matching phone. Confirm to link."
          >
            <ul style={{ margin: 0, paddingLeft: 20 }}>
              {suggestions.map((s) => (
                ...
                >
                  <span style={{ color: theme.text }}>
                    {s.channel_type} — {s.channel_display_name ?? s.external_user_id}
                  </span>
            ...
            <ul style={{ margin: 0, paddingLeft: 20 }}>
              {data.orders.map((o) => (
                <li key={o.id} style={{ marginBottom: 6 }}>
                  <a href={`/merchant/orders/${o.id}`} style={{ color: theme.primary }}>
                    Order {o.id.slice(0, 8)}…
                  </a>{' '}
                  <span style={{ color: theme.textSecondary }}>
                    {o.status} · {o.payment_status} · {o.amount}
                  </span>
                </li>
              ))}
            </ul>
              ...
              {data.recentMessages.slice(0, 10).map((m) => (
                ...
                >
                  <span style={{ fontSize: 11, color: theme.textMuted }}>
                    {m.channel_type} · {m.direction} · {new Date(m.created_at).toLocaleString()}
                  </span>
                  <div style={{ color: theme.text, marginTop: 4 }}>
                    {m.text_content?.slice(0, 200) ?? '(media)'}
                  </div>
                </li>
              ))}
            </ul>
            ...
            <ul style={{ margin: 0, paddingLeft: 20, fontSize: 13, color: theme.textSecondary }}>
              {data.identityEvents.map((ev) => (
                <li key={ev.id} style={{ marginBottom: 4 }}>
                  {ev.event_type} by {ev.actor_type} at {new Date(ev.created_at).toLocaleString()}
                  {ev.reason ? ` — ${ev.reason}` : ''}
                </li>
              ))}
            </ul>
