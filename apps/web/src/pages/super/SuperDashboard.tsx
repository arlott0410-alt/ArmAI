    ...
    <PageShell title="Overview" description="AI SaaS Command Center">
      ...
      >
        <StatCard
          ...
          value={kpis ? `$${kpis.mrrThisMonth}` : `$${data.mrr}`}
          accent
        />
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 16 }}>
              <Card>
                <CardBody>
                  ...
                  <div style={{ fontSize: 22, fontWeight: 600, color: theme.highlight }}>
                    ${revenue.currentMonthMRR}
                  </div>
                </CardBody>
              </Card>
              <Card>
                  ...
                  <CardBody>
                    <ul style={{ margin: 0, paddingLeft: 18 }}>
                      {billingHealth.overdue.slice(0, 5).map((m) => (
                        <li key={m.merchantId} style={{ marginBottom: 6 }}>
                          <Link to={`/super/merchants/${m.merchantId}`}>{m.name}</Link>
                        </li>
                      ))}
                    </ul>
                  </CardBody>
                    ...
                    .map((s) => (
                      ...
                      >
                        <td style={{ padding: '12px 16px' }}>
                          <Link to={`/super/merchants/${s.merchantId}`}>{s.name}</Link>
                        </td>
