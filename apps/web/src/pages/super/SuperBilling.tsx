            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
              <thead>
                <tr style={{ textAlign: 'left', borderBottom: `1px solid ${theme.borderMuted}` }}>
                  ...
                  <th style={{ padding: '12px 16px', color: theme.textMuted, fontWeight: 600, fontSize: 11, textTransform: 'uppercase' }}>Date</th>
                </tr>
              </thead>
              <tbody>
                {events.slice(0, 50).map((ev) => {
                  ...
                  return (
                    <tr key={String(e.id)} style={{ borderBottom: `1px solid ${theme.borderMuted}` }}>
                      <td style={{ padding: '12px 16px' }}>{String(e.merchant_id ?? '—').slice(0, 8)}…</td>
                      <td style={{ padding: '12px 16px' }}>{String(e.event_type ?? '—')}</td>
                      <td style={{ padding: '12px 16px' }}>{Number(e.amount ?? 0)} {String(e.currency ?? '')}</td>
                      <td style={{ padding: '12px 16px' }}>{String(e.status ?? '—')}</td>
                      <td style={{ padding: '12px 16px' }}>{e.created_at ? new Date(String(e.created_at)).toLocaleDateString() : '—'}</td>
                    </tr>
                  );
                })}
              </tbody>
