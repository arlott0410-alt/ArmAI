  ...
  const columns = [
    {
      ...
      render: (m: MerchantListItem) => (
        <Link to={`/super/merchants/${m.id}`} style={{ fontWeight: 600, color: theme.highlight }}>
          {m.name}
        </Link>
      ),
    },
    { key: 'slug', header: 'Slug', render: (m: MerchantListItem) => m.slug },
    {
      key: 'admin_email',
      header: 'Admin email',
      render: (m: MerchantListItem) => m.admin_email ?? '—',
    },
    ...
    },
    {
      key: 'billing_status',
      header: 'Billing',
      render: (m: MerchantListItem) => <StatusBadge status={m.billing_status} />,
    },
    {
      key: 'next_billing_at',
      header: 'Next billing',
      render: (m: MerchantListItem) =>
        m.next_billing_at ? new Date(m.next_billing_at).toLocaleDateString() : '—',
    },
    {
      key: 'setup_percent',
      header: 'Setup %',
      render: (m: MerchantListItem) => `${m.setup_percent ?? 0}%`,
    },
    {
      key: 'product_count',
      header: 'Products',
      render: (m: MerchantListItem) => String(m.product_count ?? 0),
    },
    {
      key: 'payment_account_count',
      header: 'Payments',
      render: (m: MerchantListItem) => String(m.payment_account_count ?? 0),
    },
    {
      key: 'actions',
      header: 'Actions',
      render: (m: MerchantListItem) => (
        <>
          <Link to={`/super/merchants/${m.id}`} style={{ marginRight: 12, color: theme.primary }}>
            View
          </Link>
          <Link to={`/super/support?merchantId=${m.id}`} style={{ color: theme.primary }}>
            Support
          </Link>
        </>
      ),
    ...
    >
      {addOpen && (
        <Card style={{ marginBottom: 24 }}>
          <CardBody>
            <h3 style={{ marginTop: 0, marginBottom: 16, color: theme.text, fontSize: 15 }}>
              Add Merchant
            </h3>
            <form onSubmit={handleAdd}>
              ...
              <div style={{ marginBottom: 12 }}>
                ...
                <input
                  ...
                  placeholder="e.g. THB, LAK (leave empty to use country default)"
                  style={{ width: '100%', maxWidth: 320, padding: 10 }}
                />
              </div>
              {addError && <p style={{ color: theme.danger, marginBottom: 8 }}>{addError}</p>}
              <button type="submit" disabled={addLoading} style={primaryBtn}>
                Create
              </button>
              <button
                type="button"
        ...
        <Card>
          <CardBody>
            <EmptyState
              ...
              description="Add a merchant to get started."
              action={
                <button onClick={() => setAddOpen(true)} style={primaryBtn}>
                  Add Merchant
                </button>
