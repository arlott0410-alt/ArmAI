...
app.post('/:orderId/payment-method/switch', async (c) => {
  ...
  const desiredMethod = (body.desired_method ?? 'prepaid_bank_transfer') as
    | 'prepaid_bank_transfer'
    | 'prepaid_qr'
    | 'cod'
  const requestedBy = (body.requested_by ?? 'merchant_admin') as
    | 'customer'
    | 'ai'
    | 'merchant_admin'
    | 'system'
  ...
  const { allCodAllowed, anyRequiresManualCod } =
    await paymentMethodSwitch.getOrderProductsCodEligibility(supabase, merchantId, orderId)
  ...
  if (
    can.result === PAYMENT_SWITCH_RESULT.REQUIRES_MANUAL_CONFIRMATION &&
    body.requested_by !== 'merchant_admin'
  ) {
    await orderDetail.recordPaymentMethodSwitch(supabase, {
      merchantId,
      orderId,
  ...
  if (desiredMethod === PAYMENT_METHOD.COD && cod) {
    await orderDetail.switchOrderToCod(
      supabase,
      merchantId,
      orderId,
      cod,
    ...
    const account = await paymentAccounts.selectAccountForOrder(supabase, merchantId, {
      totalAmount: Number(order.amount ?? 0),
    })
    if (!account) return c.json({ error: 'No payment account available' }, 400)
    await orderDetail.switchOrderToPrepaid(
      supabase,
      merchantId,
      orderId,
      account.id,
...
app.post('/:orderId/cod/confirm', async (c) => {
  ...
  await supabase
    .from('order_cod_details')
    .update({
      cod_status: ORDER_COD_STATUS.READY_TO_SHIP,
...
app.post('/:orderId/cod/mark-collected', async (c) => {
  ...
  const itemsSummary = (items ?? [])
    .map(
      (i: { product_name_snapshot: string; quantity: number }) =>
        `${i.product_name_snapshot} x${i.quantity}`
    )
    .join(', ')
    .slice(0, 200)
  telegram
    .notifyOrderPaidToTelegram(supabase, {
      merchantId,
      orderId,
...
app.post('/:orderId/shipments', async (c) => {
  ...
  const { data: settings } = await supabase
    ...
    .select('auto_send_shipping_confirmation')
    .eq('merchant_id', merchantId)
    .single()
  if (settings?.auto_send_shipping_confirmation) {
    try {
      await fulfillment.sendShippingConfirmation(
        supabase,
        { merchantId, orderId, shipmentId: shipment.id },
        c.env
      )
    } catch {
      // non-fatal
