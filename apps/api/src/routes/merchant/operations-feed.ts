...
app.get('/feed', async (c) => {
  ...
  const [events, ambiguousImages, awaitingImages] = await Promise.all([
    supabase
      ...
      .select(
        'id, merchant_id, related_order_id, related_shipment_image_id, event_type, event_note, actor_type, actor_id, created_at'
      )
    ...
    supabase
      ...
      .select(
        'id, merchant_id, order_id, source, processing_status, created_at, telegram_message_id'
      )
      .eq('merchant_id', merchantId)
