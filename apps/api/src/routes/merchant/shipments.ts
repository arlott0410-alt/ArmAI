...
app.patch('/:shipmentId', async (c) => {
  ...
  const shipment = await fulfillment.updateShipment(supabase, {
    merchantId,
    shipmentId,
    body: parsed.data,
  })
...
app.post('/:shipmentId/send-confirmation', async (c) => {
  ...
  const result = await fulfillment.sendShippingConfirmation(
    supabase,
    { merchantId, orderId: ship.order_id, shipmentId },
    c.env
  )
