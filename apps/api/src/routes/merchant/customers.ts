...
app.get('/:id', async (c) => {
  ...
  const [identities, orderCount, recentMessages, events] = await Promise.all([
    supabase
      ...
      .select(
        'id, channel_type, external_user_id, channel_display_name, phone_number, first_seen_at, last_seen_at'
      )
...
app.patch('/:id', async (c) => {
  ...
  if (parsed.data.primary_display_name !== undefined)
    update.primary_display_name = parsed.data.primary_display_name
  if (parsed.data.phone_number !== undefined) {
    update.phone_number = parsed.data.phone_number
    update.normalized_phone = await customerIdentity.normalizePhoneForMerchant(
      supabase,
      merchantId,
