...
import { runAccountScoping, loadPaymentAccountForScoping } from '../../services/account-scoping.js'
...
app.get('/setup', async (c) => {
  ...
  const [settingsRow, configRow, paymentAccounts, lastTx, countResult, scopeCounts] =
    await Promise.all([
      supabase
        .from('merchant_settings')
        .select('bank_parser_id, webhook_verify_token')
        .eq('merchant_id', merchantId)
        .single(),
      supabase
        .from('bank_configs')
        .select(
          'id, bank_code, payment_account_id, device_label, is_active, last_tested_at, parser_id, match_mode'
        )
        .eq('merchant_id', merchantId)
  ...
  const scopedCount = scopeRows.filter(
    (r: { scope_status: string | null }) => r.scope_status === 'scoped'
  ).length
  const ambiguousCount = scopeRows.filter(
    (r: { scope_status: string | null }) => r.scope_status === 'ambiguous'
  ).length
  const outOfScopeCount = scopeRows.filter(
    (r: { scope_status: string | null }) => r.scope_status === 'out_of_scope'
  ).length
...
app.patch('/setup', async (c) => {
  ...
  if (webhookVerifyToken !== undefined)
    settingsPayload.webhook_verify_token = webhookVerifyToken?.trim() || null
  if (Object.keys(settingsPayload).length > 3) {
    const { error } = await supabase
      .from('merchant_settings')
      .upsert(settingsPayload, { onConflict: 'merchant_id' })
    if (error) return c.json({ error: error.message }, 400)
  }
...
app.get('/events', async (c) => {
  ...
  const { data: transactions } = await supabase
    .from('bank_transactions')
    .select(
      'id, amount, sender_name, transaction_at, reference_code, scope_status, scope_confidence, ignored_reason, payment_account_id, created_at'
    )
    .eq('merchant_id', merchantId)
    .order('transaction_at', { ascending: false })
    .limit(limit)
...
app.get('/processing-logs', async (c) => {
  ...
  const { data: logs } = await supabase
    ...
    .select(
      'id, raw_event_id, parser_profile_id, payment_account_id, bank_transaction_id, parse_status, scope_status, matching_eligibility, decision_reason, detail_json, created_at'
    )
    .eq('merchant_id', merchantId)
    .order('created_at', { ascending: false })
    .limit(limit)
...
app.post('/test', async (c) => {
  ...
  const [settingsRow, configRow] = await Promise.all([
    ...
    supabase
      .from('bank_configs')
      .select('id, parser_id, payment_account_id, bank_code, match_mode')
      .eq('merchant_id', merchantId)
      .limit(1)
      .maybeSingle(),
  ])
  ...
  const parserId =
    settingsRow.data?.bank_parser_id ??
    configRow.data?.parser_id ??
    getParserIdForBankCode('GENERIC')
  ...
  if (!hasToken && !useScopingTest) {
    ...
    result.messages = [
      'Generate or paste a verification token in Step 2.',
      'Then paste it into your Android notification app.',
    ]
    await updateLastTested(supabase, merchantId)
    return c.json({ ...result, messages: result.messages })
  }
  ...
  const payloadToParse = useScopingTest
    ? samplePayload
    : {
        amount: 100.5,
  ...
  try {
    ...
    if (!useScopingTest) {
      ...
      result.messages.push(
        'Connection is ready. Use the webhook URL and token in your Android app.'
      )
    }
  } catch (e) {
