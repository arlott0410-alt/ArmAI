...
app.patch('/', async (c) => {
  ...
  if (parsed.data.telegram_group_id !== undefined)
    update.telegram_group_id = parsed.data.telegram_group_id
  if (parsed.data.telegram_group_title !== undefined)
    update.telegram_group_title = parsed.data.telegram_group_title
  if (parsed.data.is_active !== undefined) update.is_active = parsed.data.is_active
  if (parsed.data.bot_token_encrypted_or_bound_reference !== undefined) {
    update.bot_token_encrypted_or_bound_reference =
      parsed.data.bot_token_encrypted_or_bound_reference
  }
  ...
  if (existing) {
    ...
    const { error } = await supabase.from('telegram_connections').insert({
      ...
      bot_token_encrypted_or_bound_reference:
        parsed.data.bot_token_encrypted_or_bound_reference ?? null,
...
app.get('/admins', async (c) => {
  ...
  const { data, error } = await supabase
    ...
    .select(
      'id, merchant_id, telegram_user_id, telegram_username, telegram_display_name, role, is_active, created_at, updated_at'
    )
    .eq('merchant_id', merchantId)
...
app.patch('/admins/:id', async (c) => {
  ...
  if (parsed.data.telegram_username !== undefined)
    update.telegram_username = parsed.data.telegram_username
  if (parsed.data.telegram_display_name !== undefined)
    update.telegram_display_name = parsed.data.telegram_display_name
