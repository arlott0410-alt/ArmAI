...
const updateSchema = {
  ...
  cod_min_order_amount: (v: unknown) =>
    typeof v === 'number' && v >= 0 ? v : v === null ? null : undefined,
  cod_max_order_amount: (v: unknown) =>
    typeof v === 'number' && v >= 0 ? v : v === null ? null : undefined,
...
app.patch('/', async (c) => {
  ...
  const supabase = getSupabaseAdmin(c.env)
  const merchantId = c.get('merchantId')
  await codSettings.upsertMerchantCodSettings(
    supabase,
    merchantId,
    update as Partial<codSettings.MerchantCodSettingsRow>
  )
  const settings = await codSettings.getMerchantCodSettings(supabase, merchantId)
