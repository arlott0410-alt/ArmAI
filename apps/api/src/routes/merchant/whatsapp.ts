...
app.get('/', async (c) => {
  ...
  const { data: connections, error } = await supabase
    ...
    .select(
      'id, merchant_id, phone_number_id, waba_id, business_account_name, is_active, webhook_verify_token, created_at, updated_at'
    )
...
app.patch('/:id', async (c) => {
  ...
  if (parsed.data.phone_number_id !== undefined)
    update.phone_number_id = parsed.data.phone_number_id
  if (parsed.data.waba_id !== undefined) update.waba_id = parsed.data.waba_id
  if (parsed.data.business_account_name !== undefined)
    update.business_account_name = parsed.data.business_account_name
  if (parsed.data.access_token_reference !== undefined)
    update.access_token_reference = parsed.data.access_token_reference
...
app.post('/test', async (c) => {
  ...
  if (!conn?.phone_number_id) {
    return c.json(
      { error: 'No active WhatsApp connection. Add a connection and set access token.' },
      400
    )
  }
  return c.json({
    ok: true,
    message:
      'WhatsApp connection is configured. Send a message to your business number from WhatsApp to start a session; replies will be delivered via the webhook.',
  })
})
