...
app.get('/', async (c) => {
  ...
  let q = supabase
    ...
    .select(
      'id, merchant_id, merchant_customer_id, channel_type, external_user_id, channel_display_name, phone_number, last_seen_at, created_at'
    )
...
app.get('/suggestions', async (c) => {
  ...
  const { data: identities } = await supabase
    .from('customer_channel_identities')
    .select(
      'id, channel_type, external_user_id, channel_display_name, normalized_phone, last_seen_at'
    )
    .eq('merchant_id', merchantId)
