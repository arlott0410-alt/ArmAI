...
app.post('/merchants', async (c) => {
  ...
  try {
    ...
    const planCurrency = getMerchantDefaultCurrency(
      parsed.data.default_currency,
      parsed.data.default_country
    )
...
app.get('/merchants/:id', async (c) => {
  ...
  const supabase = getSupabaseAdmin(c.env)
  try {
    const [merchant, plan, settings, notes, billingEvents, orderSummary, supportHistory] =
      await Promise.all([
        ...
        supabase
          ...
          .select('id, actor_id, started_at, ended_at')
          .eq('merchant_id', merchantId)
...
app.patch('/merchants/:id', async (c) => {
  ...
  if (planUpdate.success) {
    ...
    await supportService.logAudit(supabase, {
      ...
      resourceType: 'merchant',
      resourceId: merchantId,
      details: { plan: planUpdate.data },
    })
  }
  if (
    typeof body.billing_status === 'string' &&
    ['active', 'past_due', 'trialing', 'cancelled'].includes(body.billing_status)
  ) {
    await supabase
      .from('merchants')
      .update({ billing_status: body.billing_status, updated_at: new Date().toISOString() })
      .eq('id', merchantId)
    await supabase
      .from('merchant_plans')
      .update({ billing_status: body.billing_status, updated_at: new Date().toISOString() })
...
app.post('/merchants/:id/notes', async (c) => {
  ...
  const supabase = getSupabaseAdmin(c.env)
  const auth = c.get('auth')
  const note = await billingService.addInternalNote(
    supabase,
    merchantId,
    auth.userId,
    parsed.data.note
...
app.get('/channel-metrics', async (c) => {
  ...
  const merchantIds = new Set(
    (waRows.data ?? []).map((r: { merchant_id: string }) => r.merchant_id)
  )
  return c.json({
