...
export const authMiddleware = createMiddleware<{
  Bindings: Env
  Variables: { auth: AuthContext; correlationId: string }
}>(async (c, next) => {
  ...
  try {
    ...
    if (error || !user) {
      return c.json(
        { error: 'Invalid or expired token', code: 'INVALID_TOKEN', correlationId: correlation },
        401
      )
    }
 ...
 */
export const requireSuperAdmin = createMiddleware<{ Variables: { auth: AuthContext } }>(
  async (c, next) => {
 ...
 */
export const resolveMerchant = createMiddleware<{
  Variables: { auth: AuthContext; merchantId: string }
}>(async (c, next) => {
  const auth = c.get('auth')
 ...
 */
export const requireMerchantAdmin = createMiddleware<{
  Variables: { auth: AuthContext; merchantId: string }
}>(async (c, next) => {
  const auth = c.get('auth')
