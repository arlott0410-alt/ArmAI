...
export async function canSwitchPaymentMethod(
  ...
  const { order, desiredMethod, codSettings, orderAmount, allProductsCodAllowed, anyProductRequiresManualCod } = params;

  if (order.payment_method_locked_at) {
    return { result: PAYMENT_SWITCH_RESULT.DENIED, reason: 'Payment method is locked (e.g. payment already confirmed).' };
  }
  ...
  if (desiredMethod === order.payment_method) {
    return { result: PAYMENT_SWITCH_RESULT.DENIED, reason: 'Order already uses this payment method.' };
  }

  if (desiredMethod === PAYMENT_METHOD.COD) {
    if (!codSettings?.enable_cod) {
      return { result: PAYMENT_SWITCH_RESULT.DENIED, reason: 'COD is not enabled for this merchant.' };
    }
    if (!allProductsCodAllowed) {
      return { result: PAYMENT_SWITCH_RESULT.DENIED, reason: 'One or more items in this order are not eligible for COD.' };
    }
    const min = codSettings.cod_min_order_amount ?? 0;
    const max = codSettings.cod_max_order_amount;
    if (orderAmount < min) {
      return { result: PAYMENT_SWITCH_RESULT.DENIED, reason: `Order amount is below minimum for COD (${min}).` };
    }
    if (max != null && orderAmount > max) {
      return { result: PAYMENT_SWITCH_RESULT.DENIED, reason: `Order amount exceeds maximum for COD (${max}).` };
    }
  ...
  if (desiredMethod === PAYMENT_METHOD.PREPAID_BANK_TRANSFER || desiredMethod === PAYMENT_METHOD.PREPAID_QR) {
...
export async function getOrderProductsCodEligibility(
  ...
  const anyRequiresManualCod = (products ?? []).some((p) => p.requires_manual_cod_confirmation === true);
