...
export function extractMessageChanges(
  body: WhatsAppWebhookBody
): { phoneNumberId: string; value: WhatsAppChangeValue }[] {
...
function normalizeWaMessage(msg: WhatsAppMessage): {
...
export async function ingestWhatsAppMessage(
  ...
  for (const msg of messages) {
    ...
    const { id: identityId, merchantCustomerId } =
      await customerIdentity.getOrCreateChannelIdentity(supabase, {
        merchantId: p.merchantId,
        channelType: 'whatsapp',
        externalUserId: from,
    ...
    if (linkedCustomerId) {
      await channel.setConversationMerchantCustomer(supabase, {
        merchantId: p.merchantId,
        conversationId,
        merchantCustomerId: linkedCustomerId,
      })
    }
