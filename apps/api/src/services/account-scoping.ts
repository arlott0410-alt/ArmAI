...
export function runAccountScoping(input: ScopingInput): ScopingResult {
  ...
  const expectedNormalized =
    paymentAccount.account_number_normalized ??
    normalizeAccountNumber(paymentAccount.account_number)
  const expectedSuffix =
    paymentAccount.account_suffix ??
    (paymentAccount.account_number
      ? suffixOf(paymentAccount.account_number.replace(/\D/g, ''), 4)
      : null)
  ...
  if (recvSuffix || (recvAccount && expectedSuffix)) {
    const recvS =
      recvSuffix ??
      (recvAccount
        ? suffixOf(normalizeAccountNumber(recvAccount), expectedSuffix?.length ?? 4)
        : null)
...
export async function loadPaymentAccountForScoping(
  ...
  const { data } = await supabase
    ...
    .select(
      'id, account_number, account_number_normalized, account_suffix, account_holder_name, account_aliases_json, bank_code, is_active'
    )
