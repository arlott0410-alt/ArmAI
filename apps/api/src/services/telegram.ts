...
export async function getTelegramConnection(
  ...
  const { data, error } = await supabase
    ...
    .select(
      'id, merchant_id, bot_token_encrypted_or_bound_reference, telegram_group_id, telegram_group_title, is_active, created_at, updated_at'
    )
...
export async function getMerchantTelegramSettings(
  ...
  const { data, error } = await supabase
    ...
    .select(
      'telegram_notify_order_paid, telegram_allow_shipment_confirmation, telegram_allow_ai_escalation, telegram_require_authorized_admins, telegram_auto_send_shipment_confirmation'
    )
    .eq('merchant_id', merchantId)
    .maybeSingle()
  if (error) throw new Error(error.message)
...
export async function sendAiEscalationToTelegram(
  ...
  if (
    ...
    settings?.telegram_allow_ai_escalation !== true
  ) {
    return { sent: false }
  }
  const lines = [
