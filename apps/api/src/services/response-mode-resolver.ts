...
export function resolveLowCostResponse(input: ResolverInput): ResolvedResponse {
  ...
  if (responseMode === 'retrieval') {
    ...
    if (
      ...
      return {
        mode: 'retrieval',
        text: `Your tracking number is: ${trackingNumber}. You can track your parcel with the courier.`,
        needAi: false,
      }
    }
    ...
    if (routeCategory === 'switch_to_cod') {
      return {
        mode: 'retrieval',
        text: codEnabled
          ? 'COD is available. We can switch your order to pay on delivery.'
 ...
 */
export function getFaqAnswerForQuery(
  query: string,
  ...
  for (const faq of faqs) {
    if (
      faq.question.trim().toLowerCase().includes(q) ||
      q.includes(faq.question.trim().toLowerCase())
    ) {
      return faq.answer
    }
  }
