 ...
 */
export async function purgeWebhookEvents(supabase: SupabaseClient, options?: { shortDays?: number }): Promise<number> {
 ...
 */
export async function purgeTelegramMessages(supabase: SupabaseClient, options?: { shortDays?: number }): Promise<number> {
  const cutoff = cutoffDays(options?.shortDays ?? SHORT_RETENTION_DAYS);
  const { data: rows, error } = await supabase
 ...
 */
export async function purgeBankRawEvents(supabase: SupabaseClient, options?: { shortDays?: number }): Promise<number> {
  try {
    const cutoff = cutoffDays(options?.shortDays ?? SHORT_RETENTION_DAYS);
    const { data: rows, error } = await supabase
    ...
    if (error || !rows?.length) return 0;
    const ids = rows.map((r: { id: string }) => r.id);
    const { error: delErr } = await supabase.from('bank_raw_notification_events').delete().in('id', ids);
    if (delErr) return 0;
    return ids.length;
  } catch {
    return 0;
