...
export async function processTelegramUpdate(
  ...
  const isAdmin = telegramUserId
    ? await telegram.isAuthorizedTelegramAdmin(supabase, p.merchantId, telegramUserId)
    : false
    ...
    if (fileId) {
      ...
      if (result.status === 'awaiting_order_reference') {
        ...
        if (sent.ok) {
          await supabase
            .from('telegram_messages')
            .update({ processed_status: 'needs_manual_link', updated_at: new Date().toISOString() })
            .eq('id', tm.id)
        }
      }
      await supabase
        .from('telegram_messages')
        .update({ processed_status: 'processed', updated_at: new Date().toISOString() })
        .eq('id', tm.id)
  ...
  if (messageType === 'reply' && canAct && msg.reply_to_message) {
    ...
    const linked = await shipmentImage.tryLinkFromTelegramReply(
      supabase,
      p.connection,
      chatId,
      replyText
  ...
  if (messageType === 'command') {
    ...
    if (text === '/help' || text.startsWith('/help ')) {
      ...
      await telegram.sendTelegramText(
        p.connection.bot_token_encrypted_or_bound_reference!,
        chatId,
        lines.join('\n')
      )
  ...
  await supabase
    .from('telegram_messages')
    .update({ processed_status: 'ignored', updated_at: new Date().toISOString() })
    .eq('id', tm.id)
  return { processed: true }
}
