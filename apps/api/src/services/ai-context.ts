...
export async function buildAiContext(
  ...
  if (resolveOrderId) {
    ...
    if (order) {
      const { data: orderItems } = await supabase
        .from('order_items')
        .select('product_name_snapshot, quantity, unit_price, total_price')
        .eq('order_id', resolveOrderId)
      const lines = (orderItems ?? [])
        .map((i) => `${i.product_name_snapshot} x${i.quantity} = ${i.total_price}`)
        .join('; ')
      ...
      const { data: latestShipment } = await supabase
        ...
        .select(
          'courier_name, tracking_number, tracking_url, shipped_at, shipping_note, shipment_status'
        )
        .eq('order_id', resolveOrderId)
        .eq('merchant_id', merchantId)
        .order('created_at', { ascending: false })
  ...
  if (linkedCustomerId) {
    ...
    const orderLines = (ordersByCustomer.data ?? [])
      .map((o) => `Order ${o.id}: ${o.status}, ${o.payment_status}, amount ${o.amount}`)
      .join('. ')
    ...
    unifiedCustomerSummary =
      [orderLines, lastLine].filter(Boolean).join(' ') ||
      'Unified customer has linked channel identities.'
  }

  let conversationSummaryText: string | null = null
  if (useConversationSummary && conversationId) {
    const summaryRow = await conversationSummary.getConversationSummary(
      supabase,
      merchantId,
      conversationId
    )
    if (summaryRow) {
      const parts = [
        ...
        summaryRow.latest_fulfillment_state &&
          `Fulfillment state: ${summaryRow.latest_fulfillment_state}`,
      ].filter(Boolean)
  ...
  const codSettingsShaped =
    codSettingsRow && typeof codSettingsRow === 'object' && 'enable_cod' in codSettingsRow
      ? (codSettingsRow as {
          enable_cod: boolean
