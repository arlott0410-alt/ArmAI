...
const GREETING_PATTERNS =
  /^(hi|hello|hey|sawasdee|sabaidee|สวัสดี|ສະບາຍດີ|ສະບາຍ|ดีครับ|ดีค่ะ|ดีคะ|good morning|good afternoon|good evening|สับ|ฮัลโล)[\s!.]*$/i
const SMALL_TALK =
  /^(ok|okay|thanks|thank you|ขอขอบคุณ|ได้ครับ|ครับ|ค่ะ|cool|got it|เข้าใจ)[\s!.]*$/i
...
const FAQ_KNOWLEDGE =
  /how to|where to|when|why|what is|where is|อย่างไร|ที่ไหน|เมื่อไหร่|ทำไม|คืออะไร/i
...
export function routeIncomingConversationEvent(event: NormalizedIncomingEvent): RouteResult {
  ...
  if (GREETING_PATTERNS.test(t))
    return { routeCategory: 'greeting', responseMode: 'template', templateHint: 'greeting' }
  if (SMALL_TALK.test(t))
    return { routeCategory: 'generic_small_talk', responseMode: 'template', templateHint: 'ack' }
  ...
  if (PAYMENT_SLIP.test(t))
    return {
      routeCategory: 'payment_slip_related',
      responseMode: event.activeOrderId ? 'retrieval' : 'ai',
      templateHint: 'send_slip_instructions',
    }
  if (COD_PATTERNS.test(t))
    return { routeCategory: 'switch_to_cod', responseMode: event.codEnabled ? 'retrieval' : 'ai' }
  if (PREPAID_PATTERNS.test(t))
    return { routeCategory: 'switch_to_prepaid', responseMode: 'retrieval' }
  if (PRICE_PATTERNS.test(t)) return { routeCategory: 'price_inquiry', responseMode: 'retrieval' }
  if (PRODUCT_PATTERNS.test(t))
    return { routeCategory: 'product_inquiry', responseMode: 'retrieval' }
  if (BUY_INTENT.test(t)) return { routeCategory: 'buy_intent', responseMode: 'retrieval' }
  if (PAYMENT_METHOD.test(t))
    return { routeCategory: 'payment_method_inquiry', responseMode: 'retrieval' }
  if (FAQ_KNOWLEDGE.test(t))
    return { routeCategory: 'faq_knowledge_question', responseMode: 'retrieval' }
  if (ESCALATION.test(t))
    return { routeCategory: 'request_human_escalation', responseMode: 'escalation' }
