...
export function computeMatchScore(
  ...
  if (
    slip.reference_code != null &&
    slip.reference_code !== '' &&
    bank.reference_code != null &&
    bank.reference_code !== ''
  ) {
    referenceScore =
      normalizeForSimilarity(slip.reference_code) === normalizeForSimilarity(bank.reference_code)
        ? 1
        : 0
  }
  ...
  const expectedAccount =
    (bank as { detected_account_number?: string | null; expected_account_number?: string | null })
  ...
  const bankDetected =
    (bank as { detected_account_number?: string | null }).detected_account_number ?? null
  if (expectedAccount != null && expectedAccount !== '') {
    const normalizedExpected = expectedAccount.replace(/\s/g, '')
    if (
      slipReceiver != null &&
      slipReceiver !== '' &&
      normalizedExpected === slipReceiver.replace(/\s/g, '')
    )
      receiverAccountScore = 1
    else if (bankDetected != null && normalizedExpected === bankDetected.replace(/\s/g, ''))
      receiverAccountScore = 1
    else if (
      slipReceiver != null &&
      slipReceiver !== '' &&
      bankDetected != null &&
      slipReceiver.replace(/\s/g, '') === bankDetected.replace(/\s/g, '')
    )
      receiverAccountScore = 0.8
  }
